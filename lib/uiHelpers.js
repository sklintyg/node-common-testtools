				...
				.then(function(){
					return elm.sendKeys(protractor.Key.CONTROL + "v")
						.then(function(){
							...
							return logger.silly('pasteString OK - ' + description + "Sträng: " + string + ', skickat till element med id: ' + elmId);
						}, function(reason) {
...
function scrollContainer(elmId, offset){
	...
	*/
	...
	if (elmId) {
		var script = 'if (!Element.prototype.documentOffsetTop) {';
		...
		script += 'var elm = document.getElementById("' + elmId + '"); ';
		...
		/*
		//debug
		script += 'console.log("elm.documentOffsetTop() " + elm.documentOffsetTop());';
		script += 'console.log("parent.documentOffsetTop() " + parent.documentOffsetTop());';
		script += 'console.log("parent.offsetHeight/2 " + parent.offsetHeight/2);';
		*/
		
		script += ' parent.scrollTop = elm.documentOffsetTop() - parent.documentOffsetTop() - (parent.offsetHeight/2); ';
...
function isParentScrollable(elmId, offset){
	...
	*/
	...
	/* Debug
	script += ' console.log(element.id);';
	script += ' console.log("element.scrollHeight: " + element.scrollHeight); ';
	script += ' console.log("element.clientHeight: " + element.clientHeight);';
	*/
	
	script += ' if (element.scrollHeight > element.clientHeight) { ';
