...
function scrollElement(elm, parents){
	...
	.then(function(elmId){
		if (elmId) {
			var frontEndJs = 'var elm = document.getElementById("' + elmId + '"); ';
			
			frontEndJs += 'var parent = elm';
			for (var i = 0; i < parents; i++) { 
			frontEndJs += '.parentNode';
			}
			
			frontEndJs += '; parent.scrollTop = elm.offsetTop - parent.offsetTop; ';
			
			//Debug
			//frontEndJs += 'console.log(elm.offsetTop + ", " + parent.offsetTop);';
			//console.log(frontEndJs);
			
			return browser.executeScript(frontEndJs);
		} else {
			throw('funktionen scrollElement hittade inget ID på elementet och kunde inte skrolla');
		}
